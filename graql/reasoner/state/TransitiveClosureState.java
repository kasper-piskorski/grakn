...
public class TransitiveClosureState extends ResolutionState {
    ...
    private Iterator<AnswerState> generateAnswerIterator(){
        ...
        Variable from = varPair.first();
        Variable to = varPair.second();
        ...
        return new IterativeTarjanTC<>(conceptGraph).stream()
                .map(e -> new ConceptMap(ImmutableMap.of(varPair.first(), e.first(), varPair.second(), e.second()), new Explanation(), query.getPattern()))
    ...
    public ResolutionState generateChildState() {
        ...
        CacheCasting.queryCacheCast(query.tx().queryCache()).ackCompleteness(queryToCache);
        return null;
    }
}