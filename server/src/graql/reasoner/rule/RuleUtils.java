...
public class RuleUtils {
    ...
    public static boolean subGraphIsCyclical(Set<InferenceRule> rules, TransactionOLTP tx){
        ...
        tx.profiler().updateTime(RuleUtils.class.getSimpleName() + "::subGraphIsCyclical::findTypeCycles", System.currentTimeMillis() - start);
        return typeCycles.stream().anyMatch(typeSet -> {
            ...
            boolean instanceCyclesPresent = !new TarjanSCC<>(conceptMap).getCycles().isEmpty();
            tx.profiler().updateTime(RuleUtils.class.getSimpleName() + "::subGraphIsCyclical::findInstanceCycles", System.currentTimeMillis() - start2);
            return instanceCyclesPresent;
